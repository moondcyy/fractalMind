...
@connect()
export default class AliAuth extends React.Component {
  state = {
    isAuth: null,
    count: 3,
    goto: false,
  ...
  componentDidMount() {
    const { auth_code, state } = getUrlParams();
    request({
      url: `/web/ali/auth?auth_code=${auth_code}&state=${state}`,
    }).then((res) => {
      this.setState({
        isAuth: res.success,
      });
      ...
      <div>
        ...
        { isAuth === false && (
          <Fragment>
            ...
            <p style={{ fontSize: 14 }}>
              <Link to="/user/login?platform=ali">重新扫码</Link>
            </p>
          </Fragment>
        )}
      </div>
