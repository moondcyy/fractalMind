...
*   isMyCreate: false // 是否为自己创建的
*   isShareBtn: 是否有"分享"按钮
*   isCreateBtn: 是否有"新增阶段"按钮
*   isCheckBtn: 是否有"管理阶段"按钮
*   isSignUpBtn: 是否有"报名情况"按钮
*   isEditBtn: 是否有"编辑"按钮
* }
* */
...
import CreateStageBtn from './CreateStageBtn';
import CheckStageBtn from './CheckStageBtn';
import NoData from '../../NoData';
import BlankLink from '../../BlankLink';
import styles from './index.less';
import { message } from 'antd/lib/index';

class CourseCardList extends React.PureComponent {
  state = {
    loading: false,
    data: {},
  };
  ...
  onFetch = (obj) => {
    const { fetchUrl = '/web/leaguer/train', params = {} } = this.props;
                ...
                {(isShareBtn || isCheckBtn || isSignUpBtn || isCreateBtn || isApproval || isRelease) && (
                  <div className={`${styles.item_btns} clear`}>
                    ...
                    <div className={styles.float_right}>
                      {isEditBtn && (
                        <BlankLink href={`/web/train/view?eventNo=${item.eventNo}`}>
                          <Button icon="edit">编辑</Button>
                        </BlankLink>
                      )}
                      {isShareBtn && <ShareBtn {...item} />}
                      {isCreateBtn && <CreateStageBtn eventNo={item.eventNo} />}
                      {isCheckBtn && <CheckStageBtn eventNo={item.eventNo} />}
                      {isSignUpBtn && (
                        <Link to={`/enroll?eventNo=${item.eventNo}`} target="_blank">
                          <Button icon="team">报名情况</Button>
                        </Link>
                      )}
                    </div>
                  </div>
